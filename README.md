...
Use the jar generated by `gradle build` for a normal Minecraft installation. A non-reobfuscated development jar can crash during command registration with `AbstractMethodError`.
...
通常の Minecraft 環境には、`gradle build` で生成された jar を入れてください。reobf されていない開発用 jar を入れると、コマンド登録時に `AbstractMethodError` でクラッシュすることがあります。
